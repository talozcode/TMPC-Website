                ...
                <td className="px-5 py-3 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <form action={moveCategoryOrder.bind(null, cat.id, 'up')}>
                      <button type="submit" disabled={idx === 0} className="text-gray-400 hover:text-gray-700 disabled:opacity-20 text-xs px-1 transition-colors">↑</button>
                    </form>
                    <form action={moveCategoryOrder.bind(null, cat.id, 'down')}>
                      <button type="submit" disabled={idx === (categories.length - 1)} className="text-gray-400 hover:text-gray-700 disabled:opacity-20 text-xs px-1 transition-colors">↓</button>
                    </form>
                    <form action={deleteCategory.bind(null, cat.id)}>
                      <button type="submit" className="text-red-400 hover:text-red-600 text-xs font-semibold transition-colors">
                        Delete
                      </button>
                    </form>
                  </div>
