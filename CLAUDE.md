...
> differ from training data — read `node_modules/next/dist/docs/` before writing Next code.
...
`npm run build` means a failed deploy — always build before pushing.
...
A marketing site + self-serve CMS. **Supabase is the single source of truth**; the public
...
**Auth & RLS:** `proxy.ts` gates `/admin/*` — unauthenticated → `/admin/login`, logged-in on
login → `/admin/projects`. RLS (see `supabase/migrations/001_initial_cms_schema.sql`) allows
public read of *published/active* rows; the admin escapes RLS via the service-role client.

**Data flow:** admin (`app/admin/*`, service-role client + server actions) mutates Postgres →
...
- After Edit/Write, commit and push (Vercel deploys). Branch off `main` is not required here.
...
- Renaming a UI label ≠ touching DB column / form `name=` keys — keep data keys stable to avoid
  migrations.
